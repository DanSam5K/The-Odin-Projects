...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:dansam_drink) {described_class.new('holy water')}

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(dansam_drink.type).to eq('holy water')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(dansam_drink.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:holy_drink) {described_class.new}

      # remove the 'x' before running this test
      it 'is full' do
        expect(holy_drink).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:self_drink) {described_class.new('pride', 0)}

      # remove the 'x' before running this test
      it 'describe a selfish treat' do
        expect(self_drink.type).to eq('pride')
      end
      it 'is not full' do
        expect(self_drink).not_to be_full
      end
    end
  end
end
